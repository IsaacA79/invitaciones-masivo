export const generateImage = async (id = "invitacion", filename = "invitacion.jpeg", scale = 2) => {
  ...
  if (document.fonts?.ready) {
    try { await document.fonts.ready; } catch (e) { console.log(e); }
  }
  ...
  let nodeOrig = svgOriginal, nodeClon = svgClon;
  ...
  while (nodeOrig && nodeClon) {
    ...
    if (cs) {
      ...
      for (const prop of STYLE_PROPS) {
        ...
        if (val && val !== "auto" && val !== "normal" && val !== "none") cssText += `${camelToKebab(prop)}:${val};`;
      }
      nodeClon.setAttribute("style", (nodeClon.getAttribute("style") || "") + cssText);

      const tag = nodeOrig.tagName.toLowerCase();
      if (tag === "p" || tag === "text") {
        if (cs.lineHeight) nodeClon.style.lineHeight = cs.lineHeight;
        if (cs.marginTop) nodeClon.style.marginTop = cs.marginTop;
        if (cs.marginBottom) nodeClon.style.marginBottom = cs.marginBottom;
  ...
  return { blob, file: new File([blob], filename, { type: "image/jpeg" }), width, height };
};